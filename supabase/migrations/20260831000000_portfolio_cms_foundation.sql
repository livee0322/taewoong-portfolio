-- Portfolio CMS: intentionally public read/write, without Supabase Auth.
...
create policy "public inserts published versions" on public.published_versions for insert to anon, authenticated with check (true);
create policy "public changes current marker" on public.published_versions for update to anon, authenticated using (true) with check (true);
...
grant select, insert, update (is_current) on public.published_versions to anon, authenticated;
grant usage, select on sequence public.published_versions_version_number_seq to anon, authenticated;
...
grant execute on function public.publish_portfolio() to anon, authenticated;
